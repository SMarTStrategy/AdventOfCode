...
fn main() {
    ...
    for calibration_value in elves_calibration_values {
        result_start += get_first_and_last_number_from_string_concat(&calibration_value);
    }

    print!("{}", result_start)
}

fn get_first_and_last_number_from_string_concat(text: &str) -> i32 {
   let concat_str = String::from_iter([get_first_number_from_text(text), get_last_number_from_text(text)]);
   concat_str.parse().unwrap()
}

fn get_first_number_from_text(text: &str) -> char {
    let word_numbers: HashMap<&str, char> = HashMap::from([
        ("one", '1'),
        ("two", '2'),
        ("three", '3'),
        ("four", '4'),
    ...
    let mut str_check = String::from("");
    for char in characters {
        if char.is_numeric() {
            return char
        }
        str_check.push(char);
        for (word_number, number) in &word_numbers {
            if str_check.contains(word_number) {
                return *number
            }
        }
    }
...
fn get_last_number_from_text(text: &str) -> char {
    let word_numbers: HashMap<&str, char> = HashMap::from([
        ("one", '1'),
        ("two", '2'),
        ("three", '3'),
