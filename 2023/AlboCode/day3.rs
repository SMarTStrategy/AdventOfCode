...
...........579....595...........8................................781...............................547.......................................";
    ...
    for line_string in  map_engine_input.split("\n") {
        line_chars.push(line_string.chars());
    }
    let mut y = 0;
    let mut x = 0;
    ...
    let mut may_gears: HashMap<Pos, Vec<i32>> = HashMap::new();
    for line_char in line_chars.clone() {
        for char_el in line_char {
            if char_el.is_digit(10) {
                number.push(char_el);
            }
            else {
                if !number.is_empty() {
                    if  is_symbol_around(number.clone(), x.add(-1), y, line_chars.clone(), &mut may_gears) {
                        println!("Engine part x: {}, y: {} - {} ", x, y, number);
                        parts.push(number.parse::<i32>().unwrap());
                        sum += number.parse::<i32>().unwrap();
                    } else {
                        // println!("x: {}, y: {} - {} ", x, y, number);
                    }
                    number = String::from("");
                }
            }
    ...
    println!("{:?}", may_gears);
    let mut sum_gear_power = 0;
    for may_gear in may_gears {
        if may_gear.1.len() == 2 {
            sum_gear_power += may_gear.1[0] * may_gear.1[1];
        }

    }

    println!("{:?}, {}", sum, sum_gear_power);
}


fn is_symbol_around(number: String, x: i32, y: i32, bid_vec: Vec<Chars<'_>>, may_gears: &mut HashMap<Pos, Vec<i32>>) -> bool {

    let neg_ops = number.len() as i32 * -1;
    let mut result = false;
    for i in -1..2 {

        for z in neg_ops..2 {
            let char_to_check = access_safe(x.add(z), y.add(i), bid_vec.clone());
            if char_to_check == '*' {
                match may_gears.get(&Pos::new(x.add(z), y.add(i))) {
                    Some(numbers) => {
                        let mut new_numbers = numbers.clone();
                        new_numbers.push(number.parse::<i32>().unwrap());
                        may_gears.insert(Pos::new(x.add(z), y.add(i)), new_numbers);
                    },
                    _ => {
                        let mut numbers = Vec::new();
                        numbers.push(number.parse::<i32>().unwrap());
                        may_gears.insert(Pos::new(x.add(z), y.add(i)), numbers);
                    }
                };
            }
...
fn access_safe(x: i32, y: i32, bid_vec: Vec<Chars<'_>>) -> char {
    if x < 0 {
        return '.';
    }
    if y < 0 {
    ...
    if y >= bid_vec.len().try_into().unwrap() {
        return '.'
    }
    if x >= bid_vec[0].clone().count().try_into().unwrap() {
        return '.'
    }
    let bid_vec_chars: Vec<char> = bid_vec[y as usize].clone().collect();
    return  bid_vec_chars[x as usize];
}
