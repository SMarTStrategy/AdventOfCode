...
fn main() {
 let games_input = "Game 1: 8 green; 5 green, 6 blue, 1 red; 2 green, 1 blue, 4 red; 10 green, 1 red, 2 blue; 2 blue, 3 red
...
Game 100: 2 red, 9 green, 11 blue; 13 blue, 4 red, 16 green; 8 green, 13 blue; 10 green, 1 red, 12 blue";
    ...
    let mut games: Vec<&str> = games_input.split("Game").collect();
    games.remove(0);

    let mut result = 0;
    let mut result_power = 0;

    for game in games {
        let game_split_from_id: Vec<&str> = game.split(":").collect();
        if is_game_possible(&game_split_from_id[1].replace("\n", ""), bag.clone()) {
            result += game_split_from_id[0].trim().parse::<i32>().unwrap();
        }
        let min_requirements = find_minimum_requirement_for_game(&game_split_from_id[1].replace("\n", ""));
        result_power += min_requirements.0 * min_requirements.1 * min_requirements.2;
        // println!("{} - {}", game_split_from_id[0], is_game_possible(&game_split_from_id[1].replace("\n", ""), bag.clone()));
    }
    println!("Possible games sum: {} // Power sum: {}",result, result_power);
}


fn is_game_possible(game: &str, bag: HashMap<&str, i32>) -> bool {

    let game_iterations = game.split(";");
    for game_iteration in game_iterations {
        let cubes = game_iteration.split(",");
        for cube in cubes {
            let number_of_cubes_and_color: Vec<&str> = cube.split(" ").collect();
            let number: i32 = number_of_cubes_and_color[1].parse::<i32>().unwrap();
            let bag_number: i32 = bag.get(number_of_cubes_and_color[2]).unwrap().clone();
            if bag_number < number {
                return false;
            }
        }
    }
...
fn find_minimum_requirement_for_game(game: &str) -> (i32, i32, i32) {
    let mut red = 0;
    let mut green = 0;
    let mut blue = 0;
    ...
    for game_iteration in game_iterations {
        ...
        for cube in cubes {
            let number_of_cubes_and_color: Vec<&str> = cube.split(" ").collect();
            let number: i32 = number_of_cubes_and_color[1].parse::<i32>().unwrap();
            match number_of_cubes_and_color[2].trim() {
                "red" => {
                    if number > red {
                        red = number;
                    }
