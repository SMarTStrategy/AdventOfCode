...
for row in rows:
    if row.__len__() == 0:
        continue
    start = 0
    how_many_red = row.split("red").__len__() - 1
    for i in range(red_threshold):
        i += 1
        find = row.split(' ' + str(i) + ' red').__len__() - 1
        if find != 0:
            start += find
            top_red = i
    if how_many_red > start:
        continue
    start = 0
    how_many_green = row.split("green").__len__() - 1
    for i in range(green_threshold):
        i += 1
        find = row.split(' ' + str(i) + ' green').__len__() - 1
        if find != 0:
            start += find
            top_green = i
    if how_many_green > start:
        continue

    start = 0
    how_many_blue = row.split("blue").__len__() - 1
    for i in range(blue_threshold):
        i += 1
        find = row.split(' ' + str(i) + ' blue').__len__() - 1
        if find != 0:
            start += find
            top_blue = i
    if how_many_blue > start:
        continue

    game = top_red * top_blue * top_green
    print(game)
    possible += game
