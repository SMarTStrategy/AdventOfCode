...
last_digit = None
puzzle = puzzle.split('\n')
numbers_accepted = ['1', '2', '3', '4', '5', '6', '7', '8', '9']
numbers_in_words_accepted = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']


def get_number_by_word(word):
    dictionary = {
        'one': '1',
        'two': '2',
        'three': '3',
...
for value in puzzle:
    ...
    for number_accepted in numbers_accepted:
        found_min = value.find(number_accepted)
        if found_min != -1 and (min_index is None or found_min < min_index):
            min_index = found_min
            first_digit = number_accepted
        found_max = reversedValue.find(number_accepted)
        if found_max != -1 and (max_index is None or found_max < max_index):
            max_index = found_max
            last_digit = number_accepted

    for number_in_words_accepted in numbers_in_words_accepted:
        found_min = value.find(number_in_words_accepted)
        if found_min != -1 and (found_min < min_index):
            min_index = found_min
            first_digit = get_number_by_word(number_in_words_accepted)

        found_max = reversedValue.find(number_in_words_accepted[::-1])
        if found_max != -1 and (found_max < max_index):
            max_index = found_max
            last_digit = get_number_by_word(number_in_words_accepted)

    if max_index is None:
        max_index = min_index
    else:
        max_index = value.__len__() - max_index - 1

    numbers.append(first_digit+last_digit)


totalSum = 0
for twodigit in numbers:
    integerNumber = int(twodigit)
    totalSum += integerNumber
